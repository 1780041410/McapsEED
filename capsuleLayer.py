...
class CapsLayer(object):
    ...
    def __init__(self, num_outputs_secondCaps, vec_len_secondCaps, batch_size, iter_routing,
        ...
        self.with_routing = with_routing#False
        self.layer_type = layer_type#CONV---->卷积层
        self.batch_size = batch_size#128*2=256
        self.iter_routing = iter_routing#1
        self.embedding_size = embedding_size#100
        self.sequence_length = sequence_length#3
        self.useConstantInit = useConstantInit#True
        ...
        if self.layer_type == 'CONV':
            ...
            if not self.with_routing:
                if self.useConstantInit == False:
                    ...
                    init1 = tf.constant([[[[0.1]]], [[[0.1]]], [[[-0.1]]]])#(3,1,1,1)
                    weight_init = tf.tile(init1, [1, self.filter_size, 1, self.num_filters])#(3,1,1,50)
                    W = tf.get_variable(name="W3", initializer=weight_init)

                b = tf.Variable(tf.constant(0.0, shape=[self.num_filters]), name="b")#(50)
                #input:[256.3,100,1]
                # W:    (3,1,1,50)
                conv = tf.nn.conv2d(
                    input,
                    ...
                    strides=[1, 1, 1, 1],
                    padding="VALID",
                    name="conv")#(256,1,100,50)
                # Apply nonlinearity
                conv1 = tf.nn.relu(tf.nn.bias_add(conv, b), name="relu")#(256,100,50)

                conv1 = tf.squeeze(conv1, axis=1)#(256,100,50)

                capsules =  tf.expand_dims(conv1, -1)#(256,100,50,1)
                capsules = squash(capsules)
                '''
               [batch_size, 1, num_caps, vec_len, 1] or [batch_size, num_caps, vec_len, 1]
                '''
            ...
            if self.with_routing:
                ...
                with tf.variable_scope('routing'):
                    ...
                    capsules = routing(self.input, b_IJ, batch_size=self.batch_size, iter_routing=self.iter_routing,
                                       num_caps_i=self.input.shape[1].value, num_caps_j=self.num_outputs_secondCaps,
